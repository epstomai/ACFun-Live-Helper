...
// setLiverUIDWithRetry retries on transient upstream fasthttp connection errors
// (AcFun servers occasionally close idle keep-alive conns).
func setLiverUIDWithRetry(ac *acfundanmu.AcFunLive, uid int64) (*acfundanmu.AcFunLive, error) {
	const maxAttempts = 7
	var lastErr error
	for attempt := 0; attempt < maxAttempts; attempt++ {
		newAC, err := ac.SetLiverUID(uid)
		if err == nil {
			return newAC, nil
		}
		lastErr = err
...
func (conn *wsConn) getDanmu(ctx context.Context, cancel context.CancelFunc, acMap *sync.Map, uid int64, reqID string) {
	...
	newAC, err := setLiverUIDWithRetry(aci.(*acLive).ac, uid)
	if err != nil {
