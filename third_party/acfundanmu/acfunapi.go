...
func (t *token) getManagerList() (managerList []Manager, e error) {
	...
	for i, m := range list {
		managerList[i] = parseManager(m)
	}

	return managerList, nil
}

func managerArray(v *fastjson.Value) []*fastjson.Value {
	paths := [][]string{
		{"data", "list"},
		{"data", "managerList"},
		{"data", "authorManagerList"},
		{"data", "managers"},
		{"list"},
		{"managerList"},
		{"data"},
...
func parseManager(v *fastjson.Value) Manager {
	manager := Manager{}
	if v == nil {
		return manager
	}
	parseManagerObject(v, &manager)
	for _, key := range []string{"userInfo", "authorManagerInfo", "managerInfo", "profile", "user"} {
		parseManagerObject(v.Get(key), &manager)
	}
	return manager
}

func parseManagerObject(v *fastjson.Value, manager *Manager) {
	if v == nil || manager == nil {
		return
	}
	o := v.GetObject()
	if o == nil {
	...
	o.Visit(func(k []byte, v *fastjson.Value) {
		switch string(k) {
		case "userId", "userID", "uid", "id":
			if manager.UserID == 0 {
				manager.UserID = v.GetInt64()
			}
		case "nickname", "name", "userName":
			if manager.Nickname == "" {
				manager.Nickname = string(v.GetStringBytes())
			}
		case "avatar", "headUrl", "avatarUrl", "userHeadUrl":
			if manager.Avatar == "" {
				manager.Avatar = managerAvatar(v)
			}
		case "customData":
			manager.CustomData = string(v.GetStringBytes())
		case "online":
			manager.Online = v.GetBool()
		case "userInfo", "authorManagerInfo", "managerInfo", "profile", "user":
			parseManagerObject(v, manager)
		default:
...
func managerAvatar(v *fastjson.Value) string {
	text := string(v.GetStringBytes())
	if text != "" {
		return text
	}
	if url := string(v.GetStringBytes("url")); url != "" {
		return url
	}
	if url := string(v.GetStringBytes("0", "url")); url != "" {
		return url
	}
	return ""
}
